...
int main(int argc, char **argv) {
  ...
  std::array<double, 4> wheel_pwm{0, 0, 0, 0};
  std::array<gain, 4> wheel_gain {
    gain(1, 1, 1), gain(1, 1, 1), gain(1, 1, 1), gain(1, 1, 1)
  };
  constexpr int FREQ = 100;
  constexpr int RANGE = 100;
  constexpr int MULTIPLICATION = 1;
  PID speed_pid[4] = {PID(wheel_gain[0], FREQ), PID(wheel_gain[1], FREQ),
                      PID(wheel_gain[2], FREQ), PID(wheel_gain[3], FREQ)};
  ros::Rate loop_rate(FREQ);

  RotaryInc rotary[4] = {RotaryInc(26, 19, 1), RotaryInc(6, 5, 1), RotaryInc(11, 9, 1), RotaryInc(1, 2, 3)};
  std::array<float, 4> wheel_now;
  std::array<float, 4> wheel_prev;
  std::array<float, 4> wheel_speed;
  std::array<float, 4> prev_speed;
  while (ros::ok()) {
    //各タイヤへの速度の分解
    wheelCal(wheel_pwm);
